...
namespace FSM.Runtime
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>在 <see cref="ITransition{TKey,TContext}"/> 基础上扩展了转换行为配置，提供退出时间、延迟和单次触发三项增强特性。</para>
    /// <para>通过 <see cref="Transition{TContext}"/> 的内部 Builder 构建的转换均实现此接口。</para>
    /// <para>当外部持有 <see cref="IStateMachine{TKey,TContext}.Transitions"/> 列表时，可将元素转换为此接口以访问扩展配置。</para>
    /// <code>
    /// foreach (var t in machine.Transitions)
    /// {
    ///     if (t is IExtendedTransition&lt;MyContext&gt; ext)
    ///     {
    ///         Console.WriteLine(ext.ExitTime);
    ///         Console.WriteLine(ext.Delay);
    ///         Console.WriteLine(ext.IsOneShot);
    ///     }
    /// }
    /// </code>
    /// </remarks>

...
namespace FSM.Runtime
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>实现 <see cref="IExtendContextTransition{TKey, TEvent, TContext}"/>，提供完整的转换路径配置与条件判断。</para>
    ...
    public class ContextTransition<TKey, TEvent, TContext> : IExtendContextTransition<TKey, TEvent, TContext>, IResettable
    {
        /// <summary>
        /// 源状态标识
        /// </summary>
        ...
        private ContextTransition(Func<TContext, bool> condition)
        {
        ...
        /// </summary>
        /// <remarks>
        /// <para>用于构建 <see cref="ContextTransition{TKey, TEvent, TContext}"/> 实例，通过链式调用配置转换属性。</para>
        ...
        public class Builder
        {
            ...
            /// </summary>
            /// <remarks>
            /// <para>根据当前配置生成 <see cref="ContextTransition{TKey, TEvent, TContext}"/> 实例。</para>
            /// </remarks>
            /// <returns>转换实例</returns>
            public ContextTransition<TKey, TEvent, TContext> Build()
            {
                ThrowErrorIfBuilt();
                built = true;
                return new ContextTransition<TKey, TEvent, TContext>(condition)
                {
                    FromState = fromState,
                    ToState = toState,
                    Priority = priority,

...
namespace FSM.Runtime
{
    ...
    public class ContextStateMachine<TKey, TEvent, TContext> : IContextStateMachine<TKey, TEvent, TContext>, IResettable
    {
        /// <summary>
        /// 共享上下文
        /// </summary>
        ...
        public class Builder
        {
            ...
            private readonly Dictionary<TKey, ContextStateBase<TKey, TContext>> states = new Dictionary<TKey, ContextStateBase<TKey, TContext>>();
            private readonly List<ContextStateBase<TKey, TContext>> orderedStates = new List<ContextStateBase<TKey, TContext>>();
            private readonly List<ContextTransition<TKey, TEvent, TContext>> transitions = new List<ContextTransition<TKey, TEvent, TContext>>();
            private bool built;
            ...
            public ContextStateMachine<TKey, TEvent, TContext> Build()
            {
                ...
                foreach (var t in transitions)
                {
                    if (!states.ContainsKey(t.ToState))
                        throw new StateMachineException($"Transition target state '{t.ToState}' is not registered.");

                    if (!InnerStates<TKey>.IsInnerState(t.FromState) && !states.ContainsKey(t.FromState))
                        throw new StateMachineException($"Transition source state '{t.FromState}' is not registered.");
                }

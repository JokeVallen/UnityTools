...
namespace Timer
{
    internal sealed class InnerEditorTimer
    {
        public static InnerEditorTimer Instance
        {
            get
            {
                if (Application.isPlaying)
                    throw new InvalidOperationException("[GlobalTimer] The editor timer cannot be used in PlayMode. Use the runtime timer instead.");
        ...
        private InnerEditorTimer(int capacity = 2048)
        {
        ...
        public EditorTimerHandle Register(float interval, Action callback, TimeSource source, bool loop, Optional<int> groupId)
        {
            ...
            return new EditorTimerHandle(index, slot.generation);
        }

        public void Cancel(in EditorTimerHandle handle)
        {
            ThrowErrorIfDisposed();
        ...
        public void SetPaused(in EditorTimerHandle handle, bool paused)
        {
            ThrowErrorIfDisposed();
            if (ValidateHandle(handle, out int index))
        ...
        public bool IsActive(in EditorTimerHandle handle)
        {
            ThrowErrorIfDisposed();
            return ValidateHandle(handle, out _);
        }

        public bool TryGetTimeRemaining(in EditorTimerHandle handle, out float result)
        {
            ThrowErrorIfDisposed();
            result = default;
        ...
        public bool TryGetProgress(in EditorTimerHandle handle, out float result)
        {
            ThrowErrorIfDisposed();
            result = default;
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool Reset(in EditorTimerHandle handle)
        {
            ThrowErrorIfDisposed();
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool SetInterval(in EditorTimerHandle handle, float interval)
        {
            ThrowErrorIfDisposed();
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool SetLoop(in EditorTimerHandle handle, bool loop)
        {
            ThrowErrorIfDisposed();
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool TryGetGroupId(in EditorTimerHandle handle, out int groupId)
        {
            ThrowErrorIfDisposed();
            groupId = 0;
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool TryGetInterval(in EditorTimerHandle handle, out float interval)
        {
            ThrowErrorIfDisposed();
            interval = 0f;
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool TryGetIsLoop(in EditorTimerHandle handle, out bool isLoop)
        {
            ThrowErrorIfDisposed();
            isLoop = false;
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        public bool TryGetFramesRemaining(in EditorTimerHandle handle, out float framesRemaining)
        {
            ThrowErrorIfDisposed();
            framesRemaining = 0f;
            if (!ValidateHandle(handle, out int index)) return false;
        ...
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(InnerEditorTimer));
        }
        ...
        private void OnEditorUpdate()
        {
            ...
            TickTimerSlots(TimeSchedule.Update);
        }
        ...
        private void TickTimerSlots(TimeSchedule currentSchedule)
        {
            ...
            float unscaledDelta = 0f;

            if (currentSchedule == TimeSchedule.Update || currentSchedule == TimeSchedule.Manual)
            {
                scaledDelta = (float)(EditorApplication.timeSinceStartup - lastEditorTime);
        ...
        private bool ValidateHandle(in EditorTimerHandle handle, out int index)
        {
        ...
        {
            public static readonly InnerEditorTimer instance = new InnerEditorTimer();
        }
    }
}
#endif
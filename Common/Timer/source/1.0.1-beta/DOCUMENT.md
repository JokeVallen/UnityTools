...
### 1.2 句柄结构 `TimerHandle`

表示一个计时器的唯一标识，用于后续控制。

| 成员 | 类型 | 说明 |
| :--- | :--- | :--- |
| `SlotIndex` | `int` | 内部槽位索引。 |
| `Generation` | `int` | 代际，用于验证句柄有效性。 |
...
### 1.4 可选值包装器 `Optional<T>`

用于表示可能不存在的值，常用于可选参数，支持隐式转换。
...
- **编辑器支持**：在非运行状态下调用会抛出异常，请确保仅在 PlayMode 下使用。
...
public class TimerUnitTests
{
    ...
    [UnityTest]
    public IEnumerator RegisterIndependent_WithScale_TriggersAtScaledRate()
    {
        int count = 0;
        // 2倍速，0.2秒实际间隔应等效于0.1秒，预期约3次
        var handle = GlobalTimer.RegisterIndependent(0.2f, () => count++, customScale: 2f, loop: true);
        yield return new WaitForSeconds(0.35f);
        handle.Cancel();
        Assert.That(count, Is.GreaterThanOrEqualTo(2).And.LessThan(5));
    }

    [UnityTest]
    public IEnumerator RegisterWallClock_TriggersAfterRealTimeEvenWhenPaused()
    {
        bool triggered = false;
        // 设置 Time.timeScale = 0 模拟暂停
        Time.timeScale = 0f;
        var handle = GlobalTimer.RegisterWallClock(0.2f, () => triggered = true, loop: false);
        // 使用 WaitForSecondsRealtime 确保真实时间流逝
    ...
    [UnityTest]
    public IEnumerator RegisterManual_OnlyAdvancesWhenManualUpdateCalled()
    {
        bool triggered = false;
        var handle = GlobalTimer.RegisterManual(0.1f, () => triggered = true, loop: false);
        // 不调用 ManualUpdate，等待真实时间
        yield return new WaitForSeconds(0.2f);
        Assert.IsFalse(triggered);
        // 调用一次 ManualUpdate 推进
        GlobalTimer.ManualUpdate(0.15f);
        Assert.IsTrue(triggered);
    }

    [UnityTest]
    public IEnumerator RegisterMonoFixedUnscaled_IgnoresTimeScale()
    {
        int count = 0;
        var handle = GlobalTimer.RegisterMonoFixedUnscaled(0.1f, () => count++, loop: true);
        Time.timeScale = 0.5f;
        yield return new WaitForSecondsRealtime(0.25f);
        Time.timeScale = 1f;
        handle.Cancel();
        // 因为 Time.fixedUnscaledDeltaTime 不受 timeScale 影响，应该正常计数
        Assert.That(count, Is.GreaterThanOrEqualTo(2));
    }

    [UnityTest]
    public IEnumerator RegisterCoroutineWaitForFixedUpdate_TriggersAfterFixedUpdate()
    {
        int count = 0;
        var handle = GlobalTimer.RegisterCoroutineWaitForFixedUpdate(1, () => count++, loop: true);
        yield return new WaitForSeconds(0.2f);
        handle.Cancel();
        // 至少应触发几次（物理帧频率约 0.02s）
        Assert.That(count, Is.GreaterThanOrEqualTo(5));
    }

    [UnityTest]
    public IEnumerator CancelAll_CancelsAllTimers()
    {
        bool flag1 = false, flag2 = false;
        var h1 = GlobalTimer.RegisterScaled(0.5f, () => flag1 = true, loop: false);
        var h2 = GlobalTimer.RegisterScaled(0.5f, () => flag2 = true, loop: false);
        ...
        var h3 = GlobalTimer.RegisterScaled(0.1f, () => newTrigger = true, loop: false);
        yield return new WaitForSeconds(0.15f);
    ...
    [UnityTest]
    public IEnumerator CancelAll_DoesNotAffectAlreadyPendingCallbacks()
    {
        // 注册一个短间隔循环，确保有回调已进入 pending
        int count = 0;
        var handle = GlobalTimer.RegisterScaled(0.01f, () => count++, loop: true);
        yield return new WaitForSeconds(0.05f); // 产生多个 pending
        int snapshot = count;
        GlobalTimer.CancelAll();
        // 已 pending 的回调会在本帧执行完毕，所以 count 可能已经增加
        // 但 cancelAll 不会阻止 pending 执行，但我们已经重置 pending 队列，所以不会再执行
        // 为了验证，等待一帧让 pending 处理完
        yield return null;
        // 此时 count 应该等于 snapshot（因为 CancelAll 清空了 pending）
        // 但注意：如果 CancelAll 后 pending 被清空，则不会有新的增加
        // 由于我们清空了 pending 但未执行，所以 count 不变
        Assert.AreEqual(snapshot, count);
        handle.Cancel();
    }

    [Test]
    public void CustomCombination_RegisterWithTimeDeltaAndSchedule_Works()
    {
        bool called = false;
        var handle = GlobalTimer.Register(
            ...
            schedule: TimeSchedule.Update,
            loop: false
        );
        Assert.IsFalse(handle.IsNull);
        // 由于是测试环境，无法驱动 Update，这里只验证句柄有效
        handle.Cancel();
        Assert.IsFalse(handle.IsActive());
    }

    [Test]
    public void OptionalParameters_ImplicitConversion_Works()
    {
        // 验证 bool 和 int 可隐式转换为 Optional<T>
        var handle = GlobalTimer.RegisterScaled(1f, () => { }, loop: false, groupID: 42);
        Assert.IsTrue(handle.TryGetGroupId(out int gid));
        Assert.AreEqual(42, gid);
        handle.Cancel();
    }
    ...
    [Test]
    public void CancelAll_DoesNotThrowWhenNoTimers()
    {
        Assert.DoesNotThrow(() => GlobalTimer.CancelAll());
    }

    [UnityTest]
    public IEnumerator RegisterIndependentFrame_WithScale_AdvancesFrameRateScaled()
    {
        int count = 0;
        // interval=2帧，scale=2 → 每帧推进2，所以每帧都会触发一次
        var handle = GlobalTimer.RegisterIndependentFrame(2, () => count++, customScale: 2f, loop: true);
        yield return null; // 第一帧：剩余 2 → 0，触发第一次，重置为 2
        yield return null; // 第二帧：剩余 2 → 0，触发第二次
        handle.Cancel();

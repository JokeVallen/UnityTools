    ...
    public sealed class FileLogger : LoggerBase, IDisposable, IUnityThreadLogger
    {
        ...
        protected override void DoLog(LogLevel level, string message, params object[] args)
        {
            ...
            if (config.UseAsync)
            {
                ...
                if (bufferByteSize > config.MaxFlushBufferSizeBytes)
                    FlushBufferInternal();
            }
            else
            {
        ...
        private void TryStartFlushTimerCoroutine()
        {
            ...
            if (config.CoroutineProxy == null) return;
            flushCoroutine = config.CoroutineProxy.StartCoroutine(OnTiming(config.FlushIntervalMilliseconds));
        }
        ...
        private void TryStopFlushTimerCoroutine()
        {
            if (flushCoroutine == null) return;
            if (config.CoroutineProxy == null) return;
            config.CoroutineProxy.StopCoroutine(flushCoroutine);
            flushCoroutine = null;
        }

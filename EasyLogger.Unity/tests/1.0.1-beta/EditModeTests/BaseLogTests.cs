...
public class LoggerBaseTests
{
    ...
    [Test]
    public void Log_FormatException_ShouldThrow()
    {
        var cfg = LoggerConfig.Builder.Create()
            .SetFormatter(null)
            .Build();
        var logger = new TestableLogger(cfg);
        Assert.Throws<FormatException>(() => logger.Error("Bad {0", "arg"));
    }
...
public class CompositeLoggerTests
{
    ...
    public void OneLoggerThrows_OthersStillCalled()
    {
        ...
        Assert.Throws<Exception>(() => composite.Info("test"));
        Assert.AreEqual(0, goodLogger.Logs.Count);
    }

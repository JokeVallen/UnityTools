...
namespace Orchestrator.UniTasks
{
    ...
    public sealed class UniTaskOrchestrator<TKey>
    {
        ...
        {
            public IUniTaskStep<TKey> Step { get; }
            public int[] DependencyIndices { get; }
            public IUniTaskBehavior<TKey>[] Behaviors { get; }

            public StepEntry(IUniTaskStep<TKey> step, int[] dependencyIndices, IUniTaskBehavior<TKey>[] behaviors)
            {
                Step = step;
        ...
        {
            private readonly List<IUniTaskStep<TKey>> steps;
            private readonly Dictionary<Type, List<IUniTaskBehavior<TKey>>> behaviors;
            ...
            private Builder()
            {
                steps = ListPool.Rent<IUniTaskStep<TKey>>();
                behaviors = DictionaryPool.Rent<Type, List<IUniTaskBehavior<TKey>>>();
            }
            ...
            /// <param name="behavior">行为实例</param>
            /// <returns>构建器实例</returns>
            public Builder AddBehavior<TStep1, TStep2>(IUniTaskBehavior<TKey> behavior)
            where TStep1 : IUniTaskStep<TKey>
            where TStep2 : IUniTaskStep<TKey>
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <param name="behavior">行为实例</param>
            /// <returns>构建器实例</returns>
            public Builder AddBehavior<TStep1, TStep2, TStep3>(IUniTaskBehavior<TKey> behavior)
            where TStep1 : IUniTaskStep<TKey>
            where TStep2 : IUniTaskStep<TKey>
            where TStep3 : IUniTaskStep<TKey>
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
                AddBehaviorInternal(typeof(TStep1), behavior);
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IUniTaskBehavior<TKey> behavior, Type stepType)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IUniTaskBehavior<TKey> behavior, Type stepType1, Type stepType2)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IUniTaskBehavior<TKey> behavior, Type stepType1, Type stepType2, Type stepType3)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
                AddBehaviorInternal(stepType1, behavior);
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IUniTaskBehavior<TKey> behavior, params Type[] stepTypes)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            public UniTaskOrchestrator<TKey> Build()
            {
                try
                {
                    ...
                    var sortedSteps = OrchestratorUtility.TopologicalSort(steps.Cast<IStep<TKey>>()).Cast<IUniTaskStep<TKey>>().ToArray();
                    ...
                    for (int i = 0; i < sortedSteps.Length; i++)
                    {
                        ...
                        ? list.ToArray()
                        : Array.Empty<IUniTaskBehavior<TKey>>();
                    ...
                    return new UniTaskOrchestrator<TKey>()
                    {
                        plan = plan,
                        policy = policy,
            ...
            private void AddBehaviorInternal(Type stepType, IUniTaskBehavior<TKey> behavior)
            {
                if (stepType == null) throw new ArgumentNullException(nameof(stepType));
                if (!behaviors.TryGetValue(stepType, out var list))
                {
                    list = ListPool.Rent<IUniTaskBehavior<TKey>>();

...
namespace Orchestrator.ValueTasks
{
    ...
    public sealed class ValueTaskOrchestrator<TKey>
    {
        ...
        private async ValueTask<StepResult> RunStepAsync(int index, ExecutionContext ctx, ITypedPipelineContext context, CancellationToken token)
        {
            ...
            try
            {
                var stepper = new ValueTaskBehaviorStepper<TKey>(stepEntry.Behaviors, 0, stepEntry.Step, context);
        ...
        {
            public IValueTaskStep<TKey> Step { get; }
            public int[] DependencyIndices { get; }
            public IValueTaskBehavior<TKey>[] Behaviors { get; }

            public StepEntry(IValueTaskStep<TKey> step, int[] dependencyIndices, IValueTaskBehavior<TKey>[] behaviors)
            {
                Step = step;
        ...
        {
            private readonly List<IValueTaskStep<TKey>> steps;
            private readonly Dictionary<Type, List<IValueTaskBehavior<TKey>>> behaviors;
            ...
            private Builder()
            {
                steps = ListPool.Rent<IValueTaskStep<TKey>>();
                behaviors = DictionaryPool.Rent<Type, List<IValueTaskBehavior<TKey>>>();
            }
            ...
            /// <param name="behavior">行为实例</param>
            /// <returns>构建器实例</returns>
            public Builder AddBehavior<TStep1, TStep2>(IValueTaskBehavior<TKey> behavior)
            where TStep1 : IValueTaskStep<TKey>
            where TStep2 : IValueTaskStep<TKey>
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <param name="behavior">行为实例</param>
            /// <returns>构建器实例</returns>
            public Builder AddBehavior<TStep1, TStep2, TStep3>(IValueTaskBehavior<TKey> behavior)
            where TStep1 : IValueTaskStep<TKey>
            where TStep2 : IValueTaskStep<TKey>
            where TStep3 : IValueTaskStep<TKey>
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
                AddBehaviorInternal(typeof(TStep1), behavior);
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IValueTaskBehavior<TKey> behavior, Type stepType)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IValueTaskBehavior<TKey> behavior, Type stepType1, Type stepType2)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IValueTaskBehavior<TKey> behavior, Type stepType1, Type stepType2, Type stepType3)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
                AddBehaviorInternal(stepType1, behavior);
            ...
            /// <returns>构建器实例</returns>
            public Builder AddBehavior(IValueTaskBehavior<TKey> behavior, params Type[] stepTypes)
            {
                ThrowIfBuilt();
                if (behavior == null) throw new ArgumentNullException(nameof(behavior));
            ...
            public ValueTaskOrchestrator<TKey> Build()
            {
                try
                {
                    ...
                    var sortedSteps = OrchestratorUtility.TopologicalSort(steps.Cast<IStep<TKey>>()).Cast<IValueTaskStep<TKey>>().ToArray();
                    ...
                    for (int i = 0; i < sortedSteps.Length; i++)
                    {
                        ...
                        ? list.ToArray()
                        : Array.Empty<IValueTaskBehavior<TKey>>();
                    ...
                    return new ValueTaskOrchestrator<TKey>()
                    {
                        plan = plan,
                        policy = policy,
            ...
            private void AddBehaviorInternal(Type stepType, IValueTaskBehavior<TKey> behavior)
            {
                if (stepType == null) throw new ArgumentNullException(nameof(stepType));
                if (!behaviors.TryGetValue(stepType, out var list))
                {
                    list = ListPool.Rent<IValueTaskBehavior<TKey>>();

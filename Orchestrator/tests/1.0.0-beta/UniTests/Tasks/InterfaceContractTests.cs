...
namespace Orchestrator.Tests.Tasks
{
    public class InterfaceContractTests
    {
        ...
        {
            // Arrange
            var mockStep = new Mock<IStep<string, int>>();

            // Act
            mockStep.Setup(s => s.Name).Returns("TestStep");
            mockStep.Setup(s => s.Dependencies).Returns(new List<IStep>());  // 返回 IStep 列表
        ...
        {
            // Arrange
            var mockBehavior = new Mock<IBehavior<string, int>>();
            var expectedResult = StepResult<int>.Continue(42);

            mockBehavior.Setup(b => b.HandleAsync(
                    It.IsAny<string>(),
                    It.IsAny<Func<Task<StepResult<int>>>>(),
            ...
            var result = await mockBehavior.Object.HandleAsync(
                "input",
                () => Task.FromResult(StepResult<int>.Continue(42)),

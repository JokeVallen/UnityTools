...
namespace Orchestrator.Benchmark.Tasks
{
    ...
    public class OrchestratorBenchmarks
    {
        ...
        // -------------------------------------------------------
        private Orchestrator<string, string> singleStepOrchestrator;
        private Orchestrator<string, string> parallelOrchestrator;
        private Orchestrator<string, string> diamondOrchestrator;
        private Orchestrator<string, string> behaviorOrchestrator;
        private Orchestrator<string, string> chain10NoPolicy;
        private Orchestrator<string, string> fanOut50;
        private Orchestrator<string, string> failAllOrchestrator;
        ...
        {
            // 1. 单步骤
            singleStepOrchestrator = Orchestrator<string, string>.Builder.Create()
            ...
            parallelOrchestrator = Orchestrator<string, string>.Builder.Create()
                .AddStep(stepA)
                .AddStep(stepB)
                .SetFinalStep(stepB)
                .Build();
            ...
            diamondOrchestrator = Orchestrator<string, string>.Builder.Create()
                .AddStep(root)
                .AddStep(left)
                .AddStep(right)
                .AddStep(sink)
            ...
            behaviorOrchestrator = Orchestrator<string, string>.Builder.Create()
                .AddStep(inner)
            ...
            chain10NoPolicy = Orchestrator<string, string>.Builder.Create()
            ...
            failAllOrchestrator = Orchestrator<string, string>.Builder.Create()
                .AddStep(failStep)
                .Build();
        ...
        {
            // 为了测量并发信号量的开销，构建一个简单的双步骤需要限制的场景
            var a = new SyncStep("A", "");
            var b = new SyncStep("B", "");
            var c = new SyncStep("C", "", new[] { a, b });
            var orch = Orchestrator<string, string>.Builder.Create()
    ...
    // ======================== 辅助实现 ========================
    internal sealed class SyncStep : IStep<string, string>
    {
        private readonly string _name;
        private readonly string _output;
        private readonly IReadOnlyCollection<IStep> _dependencies;

        public SyncStep(string name, string output, IReadOnlyList<IStep<string, string>> dependencies = null)
        {
            _name = name;
            _output = output;
        ...
        public Task<StepResult<string>> ExecuteAsync(string input, CancellationToken token)
        {
            ...
            return Task.FromResult(StepResult<string>.Continue(_output));
        }
    }

    internal sealed class FailingStep : IStep<string, string>
    {
        private readonly string _name;
        private readonly Exception _exception;
        private readonly IReadOnlyCollection<IStep> _dependencies;

        public FailingStep(string name, Exception ex, IReadOnlyList<IStep<string, string>> deps = null)
        {
            _name = name;
            _exception = ex;
        ...
        {
            token.ThrowIfCancellationRequested();
            return Task.FromResult(StepResult<string>.Fail(_exception));
        }
    }

    internal sealed class LoggingBehavior<TIn, TOut> : IBehavior<TIn, TOut>
    {
        public async Task<StepResult<TOut>> HandleAsync(
            TIn input, Func<Task<StepResult<TOut>>> next, CancellationToken token)
        {
            token.ThrowIfCancellationRequested();
    ...
    internal sealed class TimingBehavior<TIn, TOut> : IBehavior<TIn, TOut>
    {
        public async Task<StepResult<TOut>> HandleAsync(
            TIn input, Func<Task<StepResult<TOut>>> next, CancellationToken token)
        {

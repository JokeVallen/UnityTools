...
namespace Tests.UnitTests
{
    ...
    /// 用于测试的简单生成器实现
    /// </summary>
    public class TestStringGenerator : IGenerator<string, string>
    {
    ...
    /// </summary>
    public class DisposableTestGenerator : IGenerator<string, string>, IDisposable
    {
    ...
    /// 使用继承自 BaseGeneratorMediator 的简单具体类进行测试
    /// </summary>
    public class BaseGeneratorMediatorTests
    {
        // 简单的具体中介者实现，用于暴露基类功能
        private class TestMediator : BaseGeneratorMediator<IGenerator<string, string>>
        {
            // 手动添加生成器，用于测试
            public void AddGenerator(Type type, string templatePath, string outputPath, IGenerator<string, string> generator)
            {
        ...
        public void DisposeInstance_Disposes_IDisposable()
        {
            ...
            BaseGeneratorMediator<IGenerator<string, string>>.TestDisposeInstance(disposable); // 通过反射或继承调用，此处使用 TestMediator 间接访问
    ...
    public class InterfaceContractTests
    {
        ...
        {
            IGenerator<string, string> gen = new TestStringGenerator();
        ...
        // 内部简单实现
        private class SimpleAsyncGenerator : IGeneratorAsync<string, string>
        {

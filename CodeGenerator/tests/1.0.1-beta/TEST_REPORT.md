...
测试覆盖 `GeneratorConfigAttribute`、`BaseMediator` 以及核心接口的基本契约。
...
| `AddGenerator_And_Retrieve_By_Indexer` | 验证 `BaseMediator` 字典添加后可通过类型索引获取元数据 |
...
通过 `Measure.Method` 和 `GC()` 统计各核心环节的耗时（单位：毫秒）与内存分配（单位：KB）。基于两次独立运行的综合统计，已剔除异常值。

#### 2.1 同步生成器性能

| 测试用例 | 说明 | 平均耗时 (ms) | 中位数 (ms) | 平均 GC 分配 (KB) | 备注 |
|----------|------|---------------|-------------|-------------------|------|
| `Generate_SmallTemplate_Once` | 短模板 (5 字符) 单次生成 | **0.0068** | 0.0066 | **0** | 基准参考，几乎无开销 |
| `Generate_LargeTemplate_Repeat10` | 大模板 (1000 字符) ×10 拼接 | **1.649** | 1.511 | **9** | 字符串 `+` 导致时间与 GC 显著上升 |
| `Generate_StressTest_Repeat100` | 短模板 ×100 拼接，高强度 | **0.205** | 0.203 | **99** | 大量临时字符串分配，是主要性能瓶颈 |

**分析**：`result += template` 循环产生 O(n²) 的临时对象，重复次数越多，GC 压力越大。将拼接逻辑替换为 `StringBuilder` 可消除大部分分配，使耗时降低至微秒级。

#### 2.2 异步生成器性能

| 测试用例 | 说明 | 平均耗时 (ms) | 中位数 (ms) | 平均 GC 分配 (KB) |
|----------|------|---------------|-------------|-------------------|
| `GenerateAsync_WithoutCancellation` | 异步生成 ×10，包含 `Task.Yield` | **0.0414** | 0.0376 | **8.16** |
...
| 测试用例 | 说明 | 平均耗时 (ms) | 中位数 (ms) | 平均 GC 分配 (KB) |
|----------|------|---------------|-------------|-------------------|
| `GetTemplate_SmallSize` | 提供 100 字符模板 | **0.0362** | 0.0363 | **1.00** |
| `GetTemplate_LargeSize` | 提供 100,000 字符模板 | **0.5099** | 0.4723 | **1.02** |

**分析**：字符串分配成本随长度线性增长。对于大模板，可考虑流式加载或缓存来分担压力。GC 分配基本稳定在 1KB 左右，主要来自字符串对象本身。

#### 2.4 写入器性能

| 测试用例 | 说明 | 平均耗时 (ms) | 中位数 (ms) | 平均 GC 分配 (KB) |
|----------|------|---------------|-------------|-------------------|
| `Write_LargeContent` | 写入 10,000 字符内容 (空操作) | **0.00135** | 0.00135 | **≈0** |
...
| 测试用例 | 说明 | 平均耗时 (ms) | 中位数 (ms) | 平均 GC 分配 (KB) |
|----------|------|---------------|-------------|-------------------|
| `RunAll_FiveGenerators` | 同时运行 5 个生成器（每个重复 3 次） | **0.0183** | 0.0178 | **3.03** |
| `RunAll_TwentyGenerators_Stress` | 同时运行 20 个生成器（每个重复 2 次） | **0.0082** | 0.0089 | **2.00** |

**说明**：20 个生成器的测试因迭代次数少且单次工作量较小，出现了比 5 个更快的结果。在实际均匀负载下，执行时间应与生成器数量成线性关系。建议统一每个生成器的工作量并增加测量迭代次数以得到更一致的对比。GC 分配主要集中在生成器执行过程中的字符串操作。
...
| 维度 | 性能数据 | 评级 |
|------|----------|------|
| **同步生成（小模板）** | ~6.8 μs，零 GC | ⭐⭐⭐⭐⭐ |
| **同步生成（大模板×10）** | ~1.65 ms，9 KB | ⭐⭐⭐ |
| **同步生成（压力测试×100）** | ~0.21 ms，99 KB | ⭐⭐ |
| **异步生成** | ~41 μs，8 KB | ⭐⭐⭐⭐ |
| **模板提供（小）** | ~36 μs，1 KB | ⭐⭐⭐⭐ |
| **模板提供（大）** | ~0.51 ms，1 KB | ⭐⭐⭐ |
| **写入器** | ~1.35 μs，≈0 KB | ⭐⭐⭐⭐⭐ |
...
- **性能表现**：核心抽象层调用成本极低（微秒级，近零分配）。潜在的性能瓶颈源于 **具体实现中的字符串拼接算法**，通过标准优化方法（如 `StringBuilder`）可轻松解决。
- **内存分配**：除故意使用低效拼接外，框架自身不会引起显著的 GC 压力。异步接口的使用需要权衡其便利性与额外的状态机分配。
